...
* Data elements with a VR of **AT** must now be set with values
  acceptable to :func:`~pydicom3.tag.Tag`, and are always stored as a
  :class:`~pydicom3.tag.BaseTag`.  Previously, any Python type could be
  set.
* :meth:`BaseTag.__eq__()<pydicom3.tag.BaseTag.__eq__>` returns ``False`` rather
  than raising an exception when the operand cannot be converted to
  :class:`~pydicom3.tag.BaseTag` (:pr:`1327`)
* :meth:`DA.__str__()<pydicom3.valuerep.DA.__str__>`,
  :meth:`DT.__str__()<pydicom3.valuerep.DT.__str__>` and
  :meth:`TM.__str__()<pydicom3.valuerep.TM.__str__>` return valid DICOM
  strings instead of the formatted date and time representations
  (:issue:`1262`)
* If comparing :class:`~pydicom3.dataset.FileDataset` instances, the file
  metadata is now ignored. This makes it possible to compare a
  :class:`~pydicom3.dataset.FileDataset` object with a
  :class:`~pydicom3.dataset.Dataset` object.
* :func:`~pydicom3.pixel_data_handlers.rle_handler.rle_encode_frame` is
  deprecated and will be removed in v3.0, use
  :meth:`~pydicom3.dataset.Dataset.compress` or
  :attr:`~pydicom3.encoders.RLELosslessEncoder` instead.
* :func:`~pydicom3.filereader.read_file` is deprecated and will be removed in
  v3.0, use :func:`~pydicom3.filereader.dcmread` instead.
* :func:`~pydicom3.filewriter.write_file` is deprecated and will be removed in
  v3.0, use :func:`~pydicom3.filewriter.dcmwrite` instead.
* Data dictionaries updated to version 2021b of the DICOM Standard
* :class:`~pydicom3.dataset.Dataset` no longer inherits from :class:`dict`

Enhancements
------------

* Added a command-line interface for pydicom3.  Current subcommands are:
...
* A field containing an invalid number of bytes will result in a warning
  ...
  :attr:`~pydicom3.config.convert_wrong_length_to_UN` is set to ``True``.
...
* While reading explicit VR, a switch to implicit VR will be silently attempted
  ...
  :attr:`~pydicom3.config.assume_implicit_vr_switch` is ``True`` (default)
* New functionality to help with correct formatting of decimal strings (**DS**)

    * Added :func:`~pydicom3.valuerep.is_valid_ds` to check whether a string is
      valid as a DICOM decimal string and
      :func:`~pydicom3.valuerep.format_number_as_ds` to format a given ``float``
      or ``Decimal`` as a DS while retaining the highest possible level of
      precision
    * If :attr:`~pydicom3.config.enforce_valid_values` is set to ``True``, all
    ...
    * Added optional ``auto_format`` parameter to the init methods of
      :class:`~pydicom3.valuerep.DSfloat` and
      :class:`~pydicom3.valuerep.DSdecimal` and the :func:`~pydicom3.valuerep.DS`
...
* Added methods to construct :class:`~pydicom3.valuerep.PersonName` objects
  from individual components of names (``family_name``, ``given_name``, etc.).
  See :meth:`~pydicom3.valuerep.PersonName.from_named_components` and
  :meth:`~pydicom3.valuerep.PersonName.from_named_components_veterinary`.
...
* Ensured :func:`~pydicom3.pixel_data_handlers.util.convert_color_space` uses
...
* Added support for faster decoding (~4-5x) and encoding (~20x) of *RLE Lossless*
  *Pixel Data* via the :gh:`pylibjpeg-rle <pylibjpeg-rle>` plugin
  (:pr:`1361`, :pr:`1372`).
* Added :func:`Dataset.compress()<pydicom3.dataset.Dataset.compress>` function for
...
* Added :mod:`~pydicom3.encoders` module and the following encoders:

  * :attr:`~pydicom3.encoders.RLELosslessEncoder` with 'pydicom', 'pylibjpeg'
    and 'gdcm' plugins
* Added `read` parameter to :func:`~pydicom3.data.get_testdata_file`
...
* Allow searching :class:`~pydicom3.fileset.FileSet` s for a list of elements (:pr:`1428`)
* Added hash function to SR :class:`~pydicom3.sr.Code` (:pr:`1434`)


Fixes
-----

* Fixed pickling a :class:`~pydicom3.dataset.Dataset` instance with sequences
...
* Fixed empty **PN** elements read from file being :class:`str` rather than
  :class:`~pydicom3.valuerep.PersonName` (:issue:`1338`)
...
* Allow to deepcopy a `~pydicom3.dataset.FileDataset` object (:issue:`1147`)
...
* Fixed decoding for `FloatPixelData` and `DoubleFloatPixelData` via
  `pydicom3.pixel_data_handlers.numpy_handler` (:issue:`1457`)
