...
* Fixed :func:`~pydicom3.encaps.generate_pixel_data` not returning all available
...
* Added support for converting (60xx,3000) *Overlay Data* to a numpy ndarray
  ...
  <pydicom3.dataset.Dataset.overlay_array>` (:issue:`912`)
...
* Added :func:`~pydicom3.pixel_data_handlers.util.apply_color_lut` function for
  applying color palette LUTs (:issue:`205`)
* Added :func:`~pydicom3.pixel_data_handlers.util.apply_modality_lut` function
  for applying modality LUTs.
* Added :func:`~pydicom3.data.get_palette_files` for retrieving well-known
...
* Added optional `handler` argument to
  :func:`~pydicom3.dataset.Dataset.decompress`. This lets you specify a
  particular handler, rather than following pydicom's default order (:issue:`537`)
* Added :func:`~pydicom3.pixel_data_handlers.util.apply_voi_lut` function for
  applying VOI LUTs or windowing operations.
* Added support for (7fe0,0008) *Float Pixel Data* and (7fe0,0009) *Double
  Float Pixel Data* to :attr:`~pydicom3.dataset.Dataset.pixel_array`
  (:issue:`452`)
...
* :attr:`PixelData.is_undefined_length
  <pydicom3.dataelem.DataElement.is_undefined_length>` is now set automatically
...
* :func:`~pydicom3.encaps.get_frame_offsets` now returns whether the Basic Offset Table is empty and a list of the offsets.
