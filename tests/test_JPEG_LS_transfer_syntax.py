...
try:
    from pydicom3.pixel_data_handlers import numpy_handler
...
try:
    from pydicom3.pixel_data_handlers import pillow_handler
...
try:
    from pydicom3.pixel_data_handlers import jpeg_ls_handler
...
class Test_JPEG_LS_Lossless_transfer_syntax:
    ...
    def test_read_mr_with_numpy(self):
        pydicom3.config.pixel_data_handlers = [numpy_handler]
        msg = (
    ...
    def test_read_mr_with_pillow(self):
        pydicom3.config.pixel_data_handlers = [pillow_handler]
        msg = (
            r"Unable to decode pixel data with a transfer syntax UID of "
            r"'1.2.840.10008.1.2.4.80' \(JPEG-LS Lossless Image Compression\) "
            r"as there are no pixel data handlers available."
    ...
    def test_read_mr_with_jpeg_ls(self):
        pydicom3.config.pixel_data_handlers = [numpy_handler, jpeg_ls_handler]
        a = self.jpeg_ls_lossless.pixel_array
        b = self.mr_small.pixel_array
        a_mean = a.mean()
        b_mean = b.mean()
