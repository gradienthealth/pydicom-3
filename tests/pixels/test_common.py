"""Tests for pydicom3.pixels.common."""
...
class TestCoderBase:
    ...
    def test_add_plugin(self):
        ...
        coder.add_plugin("foo", ("pydicom3.pixels.decoders.rle", "_decode_frame"))
    ...
    def test_add_plugin_unavailable(self):
        ...
        coder.add_plugin("foo", ("pydicom3.pixels.decoders.pylibjpeg", "_decode_frame"))
        ...
        with pytest.raises(ValueError, match=msg):
            coder.add_plugin(
                "foo", ("pydicom3.pixels.encoders.pyjpegls", "_encode_frame")
            )

        # UID is supported but dependencies not met
        coder = CoderBase(JPEGLSLossless, decoder=False)
        coder.add_plugin("foo", ("pydicom3.pixels.encoders.pyjpegls", "_encode_frame"))
        assert "foo" not in coder._available
        assert "foo" in coder._unavailable
        coder.remove_plugin("foo")
        assert {} == coder._unavailable
    ...
    def test_add_plugin_function_missing(self):
        ...
        coder = CoderBase(RLELossless, decoder=True)
        msg = (
            r"module 'pydicom3.pixels.decoders.rle' has no attribute 'bad_function_name'"
        )
        with pytest.raises(AttributeError, match=msg):
            coder.add_plugin(
                "foo", ("pydicom3.pixels.decoders.rle", "bad_function_name")
            )
        assert {} == coder._available
        assert {} == coder._unavailable
    ...
    def test_remove_plugin(self):
        ...
        coder = CoderBase(RLELossless, decoder=True)
        coder.add_plugin("foo", ("pydicom3.pixels.decoders.rle", "_decode_frame"))
        coder.add_plugin("bar", ("pydicom3.pixels.decoders.rle", "_decode_frame"))
